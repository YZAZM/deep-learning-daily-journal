...
if __name__ == "__main__":
    gradient_check()             # 你已经通过了，但保留当单元测试很好
    compare_inits_with_sgd()     # 运行初始化对比实验
...
3，修改 main，让它只跑你想跑的实验
--------------------------------------------------------
c1.std=0.01明显起步慢，xavier和c3.he起步快很多，且he略优于xavier，
说明初始化尺度决定了早期信号/梯度能不能顺畅传播。0.01 太小，会导致 logits 初期太接近 0、梯度偏弱，于是 SGD 要花很久才能“推开”决策边界。
同时，toy太简单，所有差距不大。在更深网络/更难数据上，初始化差距会被放大。
--------------------------------------------------------
c2.为什么 ReLU 理论上更适合 He，但你这里 Xavier/He 差不多？网络很浅（只有 1 个 ReLU 隐层）且任务很简单
