...
def train_toy():
    np.random.seed(0)
    net = TwoLayerNet(input_size=4, hidden_size=10, output_size=3, weight_init_std=0.01, seed=0)

    # 造一份可学习的假数据
    N = 200
    x = np.random.randn(N, 4) #创建200个4维随机样本，通过真实权重矩阵生成分类标签
    ...
    for it in range(200):
        ...
        if it % 20 == 0:
            ...
            print(f"iter {it:03d} | loss {loss:.4f} | acc {acc:.3f}")
